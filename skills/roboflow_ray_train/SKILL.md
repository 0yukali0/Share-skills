...
description: Download a Roboflow COCO dataset and train a Faster RCNN object detection model using Ray Train + PyTorch Lightning on CPU, returning model.pt if accuracy meets the requirement.
...
Download a Roboflow COCO dataset, train a Faster RCNN object detection model using Ray Train + PyTorch Lightning (CPU), evaluate mAP@0.5, and return `model.pt` if the accuracy requirement is met.

## When to Use

Use this skill when the user wants to:
- Train an object detection model on a Roboflow dataset using distributed CPU training
- Fine-tune Faster RCNN on a custom COCO-format dataset with Ray Train
...
1. Execute:
   ```
   python skill_impl/roboflow_ray_train/roboflow_ray_train.py \
...
3. **Launches** Ray Train with 2 CPU workers using `TorchTrainer` + `RayDDPStrategy`
...
`train_loop_per_worker` is defined as a nested function inside `train_model` so that cloudpickle serialises it as a closure. This is required because `roboflow_ray_train.py` is **not** present in the Ray worker environment — Ray workers receive the serialised closure over the network and execute it without access to the source file.

All dataset and model classes (`COCODetectionDataset`, `ObjectDetector`) are defined locally inside `train_loop_per_worker` for the same reason.
