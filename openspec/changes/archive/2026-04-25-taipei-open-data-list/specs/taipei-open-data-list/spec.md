...
### Requirement: 下載台北市開放資料集 JSON
系統 SHALL 從 `https://data.taipei/api/frontstage/tpeod/dataset.download?format=json` 下載資料集目錄，解析每筆資料的「資料集名稱」與「主題分類」欄位。
...
系統 SHALL 透過 `flyte run --local` 執行 `taipei_open_data_list.py` 中以 `@env.task` 裝飾的函式，與 `create-mindmap-plugin` 執行方式一致。

#### Scenario: SKILL.md 執行指令正確
- **WHEN** 依 SKILL.md 步驟在 `skills/taipei-open-data-list/` 目錄執行指令
